...
class TestOpenTagTokenCreation(unittest.TestCase):
    ...
    def test_simple_attr(self):
        expected = self._cls(
            '[colorize color="#ffaa00"]', (4, 30), "colorize", (("color", "#ffaa00"),),
        )
        result = self._cls.generate_token(4, "colorize", {"color": "#ffaa00"},)
        self.assertEqual(expected, result)
    ...
    def test_is_escaped(self):
        expected = self._cls(
            r'[thing a="hello\"world!\\"]',
            (0, 27),
            "thing",
            (("a", 'hello"world!\\'),),
        )
        result = self._cls.generate_token(0, "thing", {"a": 'hello"world!\\'},)
        self.assertEqual(expected, result)
...
class TestTokenParser(unittest.TestCase):
    ...
    def _testy(self, input_str, expected_tokens):
        ...
        try:
            ...
            print("expected: {}".format(expected_tokens))
            print("actual: {}".format(actual_tokens))
            raise
    ...
    def test_open_tag_no_attrs(self):
        input_str = "[a]"
        expected_tokens = [token_parser.OpenTagToken("[a]", (0, 3), "a", ())]
        self._testy(input_str, expected_tokens)

    def test_open_tag_no_attrs_some_space(self):
        input_str = "[a ]"
        expected_tokens = [token_parser.OpenTagToken("[a ]", (0, 4), "a", ())]
        self._testy(input_str, expected_tokens)
    ...
    def test_end_tag(self):
        input_str = "[/a]"
        expected_tokens = [token_parser.CloseTagToken("[/a]", (0, 4), "a")]
        self._testy(input_str, expected_tokens)

    def test_end_tag_with_some_space(self):
        input_str = "[/a    ]"
        expected_tokens = [token_parser.CloseTagToken("[/a    ]", (0, 8), "a")]
        self._testy(input_str, expected_tokens)
    ...
    def test_invalid_open_tag(self):
        ...
        expected_tokens = [
            token_parser.BadSyntaxToken("[bananas are the best!]", (0, 23), None),
        ]
        self._testy(input_str, expected_tokens)
    ...
    def test_salvagable_invalid_open_tag_with_attrs_end_of_file(self):
        input_str = (
            r'[bananas skin="yellow" peel="\"yucky\"" my favourite is the banana!'
        )
        expected_tokens = [
            token_parser.BadSyntaxToken(
                r'[bananas skin="yellow" peel="\"yucky\""', (0, 39), None
            ),
...
class TestAttributeEdgeCases(unittest.TestCase):
    def test_square_braces_in_attr(self):
        ...
        expected_tokens = [
            token_parser.OpenTagToken(
                '[a x="this [is] [valid]"]', (0, 25), "a", (("x", "this [is] [valid]"),)
            )
        ]
    ...
    def test_more_square_braces_in_attr(self):
        ...
        expected_tokens = [
            ...
            token_parser.OpenTagToken(
                r'[code src="this [i]is[/i] \"terrible\""]',
                (6, 46),
                "code",
        ...
        expected_tokens = [
            token_parser.OpenTagToken(
                ...
                (
                    (
                        "url",
                        "http://bs.serving-sys.com/serving/adServer.bs?cn=trd&mc=click&pli=23095321&PluID=0&ord=[timestamp]",
                    ),
                ),
            )
        ]
